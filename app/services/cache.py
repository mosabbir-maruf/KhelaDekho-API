...
class CacheService:
    ...
    async def _connect_redis(self):
        ...
        # Throttle reconnection attempts when Redis is offline to prevent blocking requests
        now = time.monotonic()
        ...
        if self._redis:
            try:
                ...
                if raw:
                    ...
                    # Dynamic instantiation to avoid circular dependencies
                    from app.models import ScrapeResult, StreamResponse
                    MODEL_MAP = {
                        "scrape": ScrapeResult,
                        "stream": StreamResponse,
                    }
                    model_cls = MODEL_MAP.get(prefix)
