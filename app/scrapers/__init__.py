...
from app.utils import safe_int, safe_float, clean_text
...
def _extract_vote_data(card: Tag) -> VoteSummary | None:
    ...
    try:
        ...
        total = 0
        if "K" in total_text.upper():
            total = int(float(total_text.replace("K", "").replace("k", "").strip()) * 1000)
        else:
...
def _extract_platform_stats(soup: BeautifulSoup) -> PlatformStats:
    ...
    if views_el:
        ...
        all_views = _parse_views_text(text)
